...
@tool
def search_similar_goods(query: str,top_k: int = 3) -> str:
    """
...
@tool
def get_goods_detail(goods_id: str) -> str:
    """
    根据商品ID列表，查询完整商品详情
    Args:
...
@tool
def filter_goods(goods_list: list[dict], min_price: float = None, max_price: float = None, only_in_stock: bool = True) -> str:
    """
    拿到商品列表后，按预算、库存过滤商品
